...
public class CPUBlurThreaded implements IBlur {
	
	Bitmap temp1;
	Bitmap temp2;
	
	public Bitmap filter(int kernelSize, final Bitmap in) {
    	...
    	final float[] kernel = KernelUtil.getGaussianKernel(kernelSize);
    	long s= SystemClock.uptimeMillis();
    	final int tcount = 2;
    	LinkedList<Thread> set = new LinkedList<Thread>();
    	...
    	for (int i=0; i<tcount; i++) {
    		...
    		Thread t = new Thread(new Runnable() {
				...
				@Override
				public void run() {
					filterHorizontal(in, temp1, kernel, j, tcount);
				}
			});
    		set.add(t);
    		t.start();
    	...
    	for (int i=0; i<tcount; i++) {
    		...
    		Thread t = new Thread(new Runnable() {
				...
				@Override
				public void run() {
					filterVertical(temp1, temp2, kernel, j, tcount);
    ...
    public void filterHorizontal(Bitmap in, Bitmap out, float[] kernel, int tid, int th) {
    	...
    	float kv;
    	int pixel;
    	for (int y=tid; y<in.getHeight(); y+=th) {
    ...
    public void filterVertical(Bitmap in, Bitmap out, float[] kernel, int tid, int th) {
    	...
    	float kv;
    	int pixel;
    	for (int x=tid; x<in.getWidth(); x+=th) {
